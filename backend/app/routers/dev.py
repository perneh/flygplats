...
from app.seed_init_data import (
    seed_golf_clubs_if_empty_session,
    seed_init_courses_if_empty_session,
    seed_players_if_empty_session,
)
...
@router.post(
    ...
    summary="Wipe domain data and reload bundled init_data (courses, clubs, players)",
)
async def factory_default(session: AsyncSession = Depends(get_session)) -> dict[str, str]:
    """
    Remove all players, rounds, shots, tournaments (and related flights/scorecards), holes, courses,
    and golf clubs, then insert rows from
    ``init_data/golf_courses_25.json``, ``init_data/golf_clubs.json``, and ``init_data/golf_players.json``
    when those tables are empty (they are after the delete). Same catalog as Docker/CLI seed.
    ...
    await seed_players_if_empty_session(session)
...
@router.post(
    "/seed-players",
    status_code=status.HTTP_200_OK,
    summary="Load bundled golf_players.json when the table is empty (tests and local dev)",
)
async def seed_players(session: AsyncSession = Depends(get_session)) -> dict[str, str | bool]:
    """Same logic as the players step in ``python -m app.seed_init_data``."""
